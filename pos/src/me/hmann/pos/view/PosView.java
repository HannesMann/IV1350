...
public class PosView {
	...
	/**
	 * Runs the point-of-sale system repeatedly by simulating a cashier, with a command line interface.
	 */
	public void simulateCashier() {
		...
		try {
			...
			while(true) {
				...
				controller.addSaleObserver(consoleObserver);
				controller.addSaleObserver(fileObserver);
				...
				while (true) {
					...
					try {
						...
						if (customerPayment >= controller.getCurrentRunningTotal()) {
							break;
						} else {
							System.out.println("Payment too low. Please try again.");
						}
					} catch (NumberFormatException exception) {
				...
				System.out.print("Start another sale (Y/N)? ");
				if(!scanner.nextLine().trim().equalsIgnoreCase("Y")) {
					break;
				}
				System.out.println();
